...
// }
    ...
    public function update(Request $request)
    {
        ...
        if (($request->hasFile('postimage'))) {
            $oldimage = Customer::select('image')->where('id', $request->postId)->first()->toArray();
            $oimage = $oldimage['image'];
            if ($oimage != null) {
                Storage::delete('public/' . $oimage);}
    ...
    {   $test = Customer::where('id',$id)->first()->toArray();
        $oimage=$test['image'];
        if($oimage!=null) {
            Storage::delete('public/'.$oimage);
        }
